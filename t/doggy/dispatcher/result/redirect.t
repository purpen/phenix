...
$url= 'http://www.chinavisual.com/';
$context->putResult('url',$url);
$context->putResult('code',302);
$invocation = new Doggy_Mock_ActionInvocation($context,null,null);
$invocation->setAction(new Doggy_Mock_Action());

$result = new Doggy_Dispatcher_Result_Redirect();
$result->execute($invocation);
$headers = $response->getHeaders();
ok(in_array(array('name'=>'Location','value'=>$url),$headers),'redirect result,headers');
is($response->getHttpResponseCode(),302,'redirect,response code');
?>