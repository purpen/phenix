...
$provider = new Doggy_Auth_Authentication_SessionProvider();

$authen = $provider->createAuthentication();
ok(!$authen->isAuthenticated(),'init authentication');

$authen->setAuthenticated(true);
$provider->createAuthentication();
ok($authen->isAuthenticated(),true,'authen granted');
$provider->revoke();
$provider->createAuthentication();
ok(!$authen->isAuthenticated(),'authen revoke');
?>