 ...
 *
 */
class Sher_Core_Util_WxPay_WxPayApi
{
	/**
	 * 
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsOut_trade_noSet()) {
			throw new Sher_Core_Util_WxPay_WxPayException("缺少统一支付接口必填参数out_trade_no！");
		}else if(!$inputObj->IsBodySet()){
			throw new Sher_Core_Util_WxPay_WxPayException("缺少统一支付接口必填参数body！");
		}else if(!$inputObj->IsTotal_feeSet()) {
			throw new Sher_Core_Util_WxPay_WxPayException("缺少统一支付接口必填参数total_fee！");
		}else if(!$inputObj->IsTrade_typeSet()) {
			throw new Sher_Core_Util_WxPay_WxPayException("缺少统一支付接口必填参数trade_type！");
		}
		
		//关联参数
		if($inputObj->GetTrade_type() == "JSAPI" && !$inputObj->IsOpenidSet()){
			throw new Sher_Core_Util_WxPay_WxPayException("统一支付接口中，缺少必填参数openid！trade_type为JSAPI时，openid为必填参数！");
		}
		if($inputObj->GetTrade_type() == "NATIVE" && !$inputObj->IsProduct_idSet()){
			throw new Sher_Core_Util_WxPay_WxPayException("统一支付接口中，缺少必填参数product_id！trade_type为NATIVE时，product_id为必填参数！");
		}
		...
		$result = Sher_Core_Util_WxPay_WxPayData_WxPayResults::Init($response);
	...
	public static function refund($inputObj, $timeOut = 6)
	{
		...
		//检测必填参数
		if(!$inputObj->IsOut_trade_noSet() && !$inputObj->IsTransaction_idSet()) {
			throw new Sher_Core_Util_WxPay_WxPayException("退款申请接口中，out_trade_no、transaction_id至少填一个！");
		}else if(!$inputObj->IsOut_refund_noSet()){
			throw new Sher_Core_Util_WxPay_WxPayException("退款申请接口中，缺少必填参数out_refund_no！");
		}else if(!$inputObj->IsTotal_feeSet()){
			throw new Sher_Core_Util_WxPay_WxPayException("退款申请接口中，缺少必填参数total_fee！");
		}else if(!$inputObj->IsRefund_feeSet()){
			throw new Sher_Core_Util_WxPay_WxPayException("退款申请接口中，缺少必填参数refund_fee！");
	...
	public static function micropay($inputObj, $timeOut = 10)
	{
		...
		if(!$inputObj->IsBodySet()) {
			throw new Sher_Core_Util_WxPay_WxPayException("提交被扫支付API接口中，缺少必填参数body！");
		} else if(!$inputObj->IsOut_trade_noSet()) {
			throw new Sher_Core_Util_WxPay_WxPayException("提交被扫支付API接口中，缺少必填参数out_trade_no！");
		} else if(!$inputObj->IsTotal_feeSet()) {
			throw new Sher_Core_Util_WxPay_WxPayException("提交被扫支付API接口中，缺少必填参数total_fee！");
	...
	public static function report($inputObj, $timeOut = 1)
	{
		...
		if(!$inputObj->IsInterface_urlSet()) {
			throw new Sher_Core_Util_WxPay_WxPayException("接口URL，缺少必填参数interface_url！");
		} if(!$inputObj->IsReturn_codeSet()) {
			throw new Sher_Core_Util_WxPay_WxPayException("返回状态码，缺少必填参数return_code！");
		} if(!$inputObj->IsResult_codeSet()) {
			throw new Sher_Core_Util_WxPay_WxPayException("业务结果，缺少必填参数result_code！");
		} if(!$inputObj->IsUser_ipSet()) {
			throw new Sher_Core_Util_WxPay_WxPayException("访问接口IP，缺少必填参数user_ip！");
	...
	public static function notify($callback, &$msg)
	{
		...
		//如果返回成功则验证签名
		try {
			$result = Sher_Core_Util_WxPay_WxPayData_WxPayResults::Init($xml);
		} catch (Sher_Core_Util_WxPay_WxPayException $e){
			$msg = $e->errorMessage();
			return false;
		}
	...
	private static function reportCostTime($url, $startTimeStamp, $data)
	{
		...
		$objInput = new Sher_Core_Util_WxPay_WxPayData_WxPayReport();
		...
		} catch (Sher_Core_Util_WxPay_WxPayData_WxPayException $e){
			//不做任何处理
		}
	}
	...
	{		
		...
		if($data){
			...
			$error = curl_errno($ch);
			curl_close($ch);
			throw new Sher_Core_Util_WxPay_WxPayData_WxPayException("curl出错，错误码:$error");
		}
	}
