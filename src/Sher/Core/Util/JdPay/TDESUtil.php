 ...
 *
 */
class Sher_Core_Util_JdPay_TDESUtil
{
	/**
	 * 将元数据进行补位后进行3DES加密
	 * <p/>
	...
	public static function encrypt2HexStr($keys, $sourceData) {
		...
		// 元数据
		$source = Sher_Core_Util_JdPay_BytesUtils::getBytes ( $sourceData );
		...
		$sizeByte = Sher_Core_Util_JdPay_BytesUtils::integerToBytes ( $merchantData );
		$resultByte = array ();
		...
		$desdata = self::encrypt ( Sher_Core_Util_JdPay_BytesUtils::toStr ( $resultByte ), $keys );
		//echo ("加密后的长度:" . strlen ( $desdata ) . "<br/>");
		return Sher_Core_Util_JdPay_BytesUtils::strToHex ( $desdata );
	}
	...
	public static function decrypt4HexStr($keys, $data) {
		...
		$hexSourceData = Sher_Core_Util_JdPay_BytesUtils::hexStrToBytes ($data);
		...
		// 有效数据长度
		$dsb = Sher_Core_Util_JdPay_BytesUtils::byteArrayToInt( $dataSizeByte, 0 );
		$tempData = array ();
 		for($j = 0; $j < $dsb; $j++) {
