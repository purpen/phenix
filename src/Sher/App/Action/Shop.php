...
class Sher_App_Action_Shop extends Sher_App_Action_Base implements DoggyX_Action_Initialize {
    ...
    //添加网站meta标签
    $this->stash['page_title_suffix'] = sprintf("【%s】-%s-太火鸟商店", $product['category']['name'], $product['short_title']);
    if(!empty($product['tags_s'])){
      $this->stash['page_keywords_suffix'] = $product['tags_s'];   
    }
    $this->stash['page_description_suffix'] = sprintf("太火鸟Taihuoniao智能硬件商店提供（%s）正品行货，全国正规智能产品购买平台，包括（%s）图片、参数、硬件测评、相关产品、使用技巧等信息，购买（%s）就去太火鸟，放心又轻松。", $product['category']['name'], $product['short_title'], $product['short_title']);
