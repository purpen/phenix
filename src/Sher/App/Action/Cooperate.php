...
class Sher_App_Action_Cooperate extends Sher_App_Action_Base implements DoggyX_Action_Initialize {
    ...
    public function view(){
        ...
        //添加网站meta标签
        $this->stash['page_title_suffix'] = sprintf("%s-太火鸟智能硬件平台合作方", $cooperate['title']);
        if(!empty($try['tags'])){
          $this->stash['page_keywords_suffix'] = "太火鸟,智能硬件,智能硬件孵化平台,孵化资源,设计公司,技术开发,合作院校,创意设计,硬件研发,硬件推广";   
        }
        $this->stash['page_description_suffix'] = sprintf("%s-中国最火爆的智能硬件孵化平台-太火鸟合作方，为您提供商品定义、投资融资、硬件品控、软件体验、推广营销、渠道建设等每个环节全程全力支持。", $cooperate['title']);
