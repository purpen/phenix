...
class Sher_Api_Action_Product extends Sher_Api_Action_Base {
  ...
  public function product_category_stick(){
    ...
    // 分享内容
    $data['share_view_url'] = sprintf("%s/shop/s_view?id=%s", Doggy_Config::$vars['app.url.wap'], $product['_id']);
    if($storage_id){
        $data['share_view_url'] = sprintf("%s&storage_id=%s", $data['share_view_url'], $storage_id);  
    }
    ...
    if(!empty($asset_result['rows'])){
      foreach($asset_result['rows'] as $key=>$value){
        array_push($assets, array($value['thumbnails']['hd']['view_url']);
      }
    }
