...
echo "-------------------------------------------------\n";
echo "===============ALLIANCE BALANCE RECORD WORKER WAKE UP===============\n";
echo "-------------------------------------------------\n";
echo "Time: ".date('Y-m-d H:i:s', time())."\n";
echo "Start to Alliance balance record...\n";
...
while(!$is_end){
    ...
    for ($i=0; $i<$max; $i++) {
        ...
        if(!$ok) {
            echo "update alliance balance_record is fail! next....";
            continue;
        }
...
echo "Alliance balance record count: [$total] is OK! \n";
