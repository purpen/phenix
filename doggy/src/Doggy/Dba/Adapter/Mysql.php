 ...
 * @author night
 */
class Doggy_Dba_Adapter_Mysql extends Doggy_Dba_Adapter_Abstract {
    ...
    public function __construct($dsn){
        ...
        if ($this->enable_mysqlnd) {
            Doggy_Log_Helper::debug('***MysqlND is actived.***',__METHOD__);
        }
    }
    ...
    public function execute($sql,$vars=array()){
        if(!$this->connect()){
            throw new Doggy_Dba_Exception("Database connect failed");
        }
        $mysqli = $this->mysqli;
        if(strpos($sql,'?')!==false){
            $ok = $this->executeStatement($mysqli,$sql,$vars,false);
        }else{
            $ok=$mysqli->query($sql);
            if($mysqli->errno){
                Doggy_Log_Helper::error("SQL ERROR:".$mysqli->error.' SQL:'.$sql,__METHOD__);
                throw new Doggy_Dba_Exception('SQL ERROR:'.$mysqli->error);
            }
        }
        return $ok;
    }
     ...
     * @return string
     **/
    protected function makePageSql($sql,$size,$page){
        if($size>0){
            if($page>=1){
                $offset = ($page-1)*$size;
            }else{
     ...
     * @return mixed
     **/
    protected function executeStatement($mysqli,$sql,$vars,$result_mode=true){
        $stmt = $mysqli->prepare($sql);
        if(!$stmt){
            throw new Doggy_Dba_Exception('Prepare SQL failed:'.$mysqli->error);
        }
        if ($stmt->param_count != count($vars)) {
            throw new Doggy_Dba_Exception('The parameter holder is not same as invoke parameter array.');
        }
        $a = '';
        foreach ($vars  as $k => $v) {
            if (is_string($v)) $a .= 's';
    ...
    public function query($sql,$size=-1,$page=1,$vars=array(),$fetch_mode=MYSQLI_ASSOC){
        ...
        $sql = $this->makePageSql($sql,$size,$page);
        
        $mysqli = $this->mysqli;
        //if has statement bind variable
        if ( strpos($sql,'?')!==false ) {
            $stmt = $this->executeStatement($mysqli,$sql,$vars);
    ...
    protected function fetchResultRows($result,$fetch_mode=MYSQLI_ASSOC){
        $rows=array();
        while($row = $result->fetch_array($fetch_mode)){
            $rows[]=$row;
        }
    ...
    public function genSeq($name){
        ...
        if(!$this->connect()){
            throw new Doggy_Dba_Exception('Connection failed.');
        }
        ...
        if (!$ok) {
            try {
                ...
                Doggy_Log_Helper::error("Cannot gen sequence,cause:".$e->getMessage(),__METHOD__);
                throw new Doggy_Dba_Exception('Generate sequence failed.');
            }
        }
