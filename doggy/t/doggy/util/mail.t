...
*/
//Now, let's rock!
if (getenv('TEST_SMTP_HOST')) {
    $host = getenv('TEST_SMTP_HOST');
}
else {
    plan('skip_all','you not set TEST_SMTP_HOST environment variable!');
}

$mail_conf = array(
    'host'=> $host,
    'smtp_auth'=>false,
    'from'=>'test@chinavisual.com',
    'from_name'=>'tester',
    );
Doggy_Config::set('app.mail.default',$mail_conf);

$to = 'panfan@chinavisual.com';
$subject= '测试邮件系统';
$body = '测试链接: http://www.chinavisual.com/ ';
$ok=Doggy_Util_Mail::send($to,$subject,$body);
ok($ok,'mail send');
diag('mail has been sent,but you should check mail from your mailbox manualy!');
?>