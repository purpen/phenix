...
$context->setResponse(new Doggy_Mock_Response());
$request = new Doggy_Mock_Request();
$context->setRequest($request);

$context->putResult('path',$file);
$invocation = new Doggy_Mock_ActionInvocation($context,null,null);
$invocation->setAction(new Doggy_Mock_Action());

$result = new Doggy_Dispatcher_Result_File();
$result->execute($invocation);
$s = file_get_contents($file);
is($result->getBuffer(),$s,'file result');
is($context->getResponse()->getContentType(),'text/plain','file result: default_content_type');
try{
    $context->putResult('path','__null__.html');
    $result->execute($invocation);
    fail('file_result:not throw execption');
