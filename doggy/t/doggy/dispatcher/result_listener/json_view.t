...
*/
//Now, let's rock!
$listner = new Doggy_Dispatcher_PreResultListener_JsonView();
$invocation = Doggy_Mock_Util::createMockAcitionInvocation();
$invocation->setResultCode(Doggy_Dispatcher_Constant_Action::SMARTY);
$invocation->getInvocationContext()->putResult('json_view',true);
$invocation->getInvocationContext()->getRequest()->setIsAjaxRequest(true);

$listner->beforeResult($invocation,$invocation->getResultCode());

is($invocation->getResultCode(),Doggy_Dispatcher_Constant_Action::JSON,'json_view result listener');
?>