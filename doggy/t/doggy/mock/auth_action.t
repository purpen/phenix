...
//Now, let's rock!
{
    $action = new Doggy_Mock_AuthAction();
    $action->setResultCode('t');
    is('t',$action->deny(),'deny');
    is('t',$action->login(),'login');
    is('t',$action->logout(),'logout');
    is('t',$action->register(),'register');
}
{
    $action = new Doggy_Mock_AuthAction();
    is('t',$action->_setNextUrl('t')->getNextUrl(),'_setNextUrl');
}
?>