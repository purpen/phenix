...
$server = Doggy_Mock_Server::mock();
is('execute',$server->getActionMethod());

$server->invoke();

unset($server);
$server = Doggy_Mock_Server::mock('Doggy_Mock_Action','publish');
is('publish',$server->getActionMethod('publish'));

$server->reset();
$server->invokeInterceptor('Doggy_Dispatcher_Interceptor_Parameters');

$server->reset();

$server->addParam('ok',200);
$server->addParam('bad','bad');

$req = $server->getRequest();

is(200,$req->getParam('ok'));
is('bad',$req->getParam('bad'));

$server->addCookie('a',200);
$server->addHttpHeader('test','ok');
$server->addHttpHeader('passport_token','ok');

is(200,$req->getCookie('a'));
is('ok',$req->getHeader('Test'));
is('ok',$req->getHeader('Passport-Token'));
