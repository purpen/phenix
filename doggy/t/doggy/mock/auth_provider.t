...
//Now, let's rock!
{
    $provider = new Doggy_Mock_AuthProvider();
    $authen = $provider->createAuthentication();
    $authen->setAuthenticated(true);
    $provider->revoke($authen);
    ok(!$authen->isAuthenticated(),'setAuthenticated/revoke');    
}
{
    $provider = new Doggy_Mock_AuthProvider();
    $authen = $provider->createAuthentication();
    ok($authen instanceof Doggy_Auth_Authentication ,'createAuthentication');
    ok($authen->getAcl() instanceof Doggy_Auth_Acl ,'createAuthentication');
    
    $authen2 = new Doggy_Mock_AuthProvider();
    $provider->setAuthentication($authen2);
    ok($authen2===$provider->createAuthentication(),'createAuthentication');
