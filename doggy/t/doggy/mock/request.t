...
$req = new Doggy_Mock_Request();
$test_file = '/tmp/__foo.test';
file_put_contents($test_file,'okokokok');
$size = filesize($test_file);
$req->mockUpload('test',$test_file);
$f = $req->getUploadFiles();
ok(isset($f['test']));
is($size,$f['test']['size']);
ok($req->isUploadedFile($f['test']['tmp_name']));
$req->mockUpload('test2','unknonw.file');
$f = $req->getUploadFiles();
ok(isset($f['test2']));
is(UPLOAD_ERR_NO_FILE,$f['test2']['error']);
$req->mockUpload(array('test'=>array($test_file,'unknown')));
$f = $req->getUploadFiles();

is(2,count($f['test']['name']));
@unlink($test_file);
?>