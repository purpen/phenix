...
$test_table = 'doggy_dba_test';

$dsn = Doggy_Config::get('app.dba.default');

if (empty($dsn)) {
    plan('skip_all', 'app.dba.default not setup,please set correctly dns before run test.'); 
}
diag("test dsn:$dsn");
$dba = new Doggy_Dba_Adapter_Mysql($dsn);
...
# getTableList
{
    ...
    $table_list = $dba->getTableList();
    ok(in_array('doggy_test_t1',$table_list) && in_array('doggy_test_t2',$table_list) ,'get_table_list');
    $dba->execute('DROP TABLE doggy_test_t2');
    $table_list = $dba->getTableList();
    ok(!in_array('doggy_test_t2',$table_list),'get_table_list,refresh');
    $dba->execute('DROP TABLE doggy_test_t1');
    $table_list = $dba->getTableList();
    ok(!in_array('doggy_test_t1',$table_list),'get_table_list,refresh');
...
# FieldMetaList
{
    ...
    $fields = $dba->getFieldMetaList($test_table);
    
    is($fields,array(
        't' => array('name'=>'t','type'=>'S','length'=>50,),
        't2' => array('name'=>'t2','type'=>'N','length'=>11),
        't3' => array('name'=>'t3','type'=>'D','length'=>-1),
        't4' => array('name'=>'t4','type'=>'T','length'=>-1),
        't5' => array('name'=>'t5','type'=>'S','length'=>100),
        ),
        'getFieldMetaList');
    _clean_test_table();
}
...
{
    $i=1;
    is($dba->genSeq($test_table),$i,'genSeq ... ori');
    $i++;
    is($dba->genSeq($test_table),$i,'genSeq ... inc');
    $dba->dropSeq($test_table);
    is($dba->genSeq($test_table),1,'dropSeq');
    $dba->dropSeq($test_table);
}
