...
*/
//Now, let's rock!
Doggy_Dt::add_tag('cache_loader','Doggy_Dt_Tag_CacheLoader');

$dt = Doggy_Dt::parse_string(
    '{%cache_loader page:"t" %}{%cache cache_key:"k1" %}{{v}}{%endcache%}{%cache cache_key:"k2" %}{{v}}{%endcache%}'
    );
    
$result = $dt->render(array('v' => 1));
is($result,'11','cache render/missing');
is(count(Doggy_Dt_Tag_CacheLoader::$cache_keys),2,'put cache keys');

Doggy_Dt_Tag_CacheLoader::reset();
...
$result = $dt->render(array('v' => 1));
is($result,'11','cache render/hit');
is(count(Doggy_Dt_Tag_CacheLoader::$cache_keys),0,'cache should all hit');




Doggy_Cache_Memcached::get_cluster()->flush();
?>