...
is(doggy_dt('{%extends "title.html"%}{%block title%}my_title{%endblock%}',$option)->render(),'my_title','basic');

$tpl_dir = dirname(__FILE__).'/templates';
$option = array('searchpath'=>$tpl_dir);
$result = doggy_dt('page1.html',$option)->render(array('name'=>'doggy'));
like($result,"/2\s+my\s+title\s+is\s+doggy./");
like($result,"/ok/");
?>