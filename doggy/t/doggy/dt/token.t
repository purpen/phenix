...
*/
//Now, let's rock!
$token = new Doggy_Dt_Token('var','content',0);
$token->write('result_content');
is($token->type,'var','type');
is($token->content,'content','content');
is($token->position,0,'position');
is($token->result,'result_content','result');
?>