...
*/
//Now, let's rock!
$stream = new Doggy_Dt_TokenStream();
$t = $stream->pop();
ok(empty($t),'pop');
$stream->push('a');
is($stream->pop(),'a','push/pop');
try{
    $stream->push(NULL);
    fail('push error');
}catch(Doggy_Dt_Exception $e) {} ;

$stream->feed('block','b1',1);
$stream->feed('var','var1',2);
$stream->feed('var','var2',3);

$stream->close();
ok($stream->is_closed(),'close');
try{
    $stream->feed('var','var3',4);
    fail('close faild');
}catch(Doggy_Dt_Exception $e) {};
...
while($stream->next()){
    $t = $stream->current();
    ok($t instanceof Doggy_Dt_Token,'current/next');
    is($t->position,$p++,'current/next');
}
