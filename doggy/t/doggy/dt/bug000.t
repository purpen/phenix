...
Doggy_Dt::add_tag('if');
{
    ...
    $stream->pop();
    $t= $stream->pop();
    is($t->type,'text','textnode,0');
    is($t->content,'0','textnode,0');
    
}
{
    #this bug is caused by Doggy_Dt_Tokenizer::tokenize
    is(doggy_dt('{%if 4>3 %}0{%else%}is_failed{%endif%}')->render(),'0','bug:if not output 0');
}
?>