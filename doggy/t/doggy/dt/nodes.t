...
//Now, let's rock!
{
    $node = new Doggy_Dt_VariableNode(':test',0);
    $context = new Doggy_Dt_Context(null,Doggy_Dt_Options::merge());
    $writer = new Doggy_Dt_StreamWriter();
    $context->set('test','ok');
    $node->render($context,$writer);
    is($writer->close(),'ok','variable_node render');
}
?>