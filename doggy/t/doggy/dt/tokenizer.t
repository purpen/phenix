...
$tokenizer = new Doggy_Dt_Tokenizer($options);

$tpl ="{# this is comment #}\n{{ title }} is ok\n{% if age > 5 %}\nHello world!\n{% endif %}";
$stream = $tokenizer->tokenize($tpl);

$t = $stream->pop();
is($t->type,'comment','comment node');
diag(print_r($t,true));

$t = $stream->pop();
is($t->type,'variable','var node');
diag(print_r($t,true));

$t = $stream->pop();
is($t->type,'text','text node');
diag(print_r($t,true));

$t = $stream->pop();
is($t->type,'block','if tag begin');
diag(print_r($t,true));

$t = $stream->pop();
is($t->type,'text','textnode');
diag(print_r($t,true));

$t = $stream->pop();
is($t->type,'block','if tag end');
diag(print_r($t,true));

$t = $stream->pop();
ok(empty($t),"eof document");
