...
$logging_settings = Doggy_Config::get('app.log.default');
$class = $logging_settings['class'];

{
    $log = Doggy_LogFactory::getLog();
    ok($log instanceof $class,'factory default instance');
}
{
    $logging_settings['test_log'] = array(
        'class' => 'Doggy_Log_Console'
        );
    Doggy_Config::set('app.log.test_log',$logging_settings);
    
    $test_log = Doggy_LogFactory::getLog('test_log');
    ok($log instanceof $class,'factory instance by id');
}
