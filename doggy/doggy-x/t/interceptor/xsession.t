...
*/
//Now, let's rock!
class DoggyX_Action_DummyTest extends DoggyX_Action_Base implements DoggyX_Action_VisitorAware {
    public function check_visitor($invoke_method, $login_user,&$handle) {
    }
}


$interceptor = new DoggyX_Interceptor_XSession();


$action = new DoggyX_Action_DummyTest();

// $interceptor->init();
$_SERVER['REQUEST_URI'] = '/mine';
$server = Doggy_Mock_Server::mock('DoggyX_Action_DummyTest','execute');
$server->invokeInterceptor($interceptor);
pass('xsession/mock bootstrap');
?>