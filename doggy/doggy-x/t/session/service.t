...
$service->start_visitor_session();
ok($service->is_session_started(),'is_session_started');
$sid = $service->session_id;
ok($sid != null,'start_visitor_session/new session');

is($service->session->user_id,0,'default session data');
$service->session->user_id = 10;
$service->session->foo = 'test';

$service->stop_visitor_session();
ok(!$service->is_session_started(),'stop_visitor_session');

unset($service);
// force new instance
$service = DoggyX_Session_Service::instance();
$service->start_visitor_session($sid);
is($service->session->foo,'test','check session store');

$service->session->destroy_session($sid);
