...
*/
//Now, let's rock!
$session = new DoggyX_Session_Engine_Mongo();

$sid = $session->build_session_id();
ok(!empty($sid),'build_session_id');

ok(!$session->load_session($sid),'load null session');

$session->create_session($sid,array('u' => 5));
is($session->u,5,'create_session/__get');
$session->year = '2010';
$session->close();

$session = new DoggyX_Session_Engine_Mongo();
ok($session->load_session($sid),'close/load session');
is($session->year,'2010','session stored');

$session->close();
$session->destroy_session($sid);
ok(!$session->load_session($sid),'destroy_session');

// ttl is 5s
$session = new DoggyX_Session_Engine_Mongo();

$sid2 = $session->build_session_id();
$session->create_session($sid2,array('u' => 6));
$session->close();

diag('wait 3s for session exipred ....');
sleep(3);
$session = new DoggyX_Session_Engine_Mongo(2);
ok(!$session->load_session($sid2),'session should expired');
