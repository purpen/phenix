...
$rows = $evt_model->find($spec);
$cnt = count($rows);

echo "START TO PROCESS USER-EVENT RECORDS, TODO:$cnt ...\n";
...
// 会员等级达标升级处理
//
$balance_rows = $point_balance_model->find();
$cnt = count($balance_rows);
echo "CHECK user rank upgrading, todo: $cnt \n";
for ($i = 0; $i < $cnt; $i++) {
...
$rows = $point_record_model->find($spec);
$cnt = count($rows);

echo "Start to accounting daily record, todo: $cnt \n";
